...
class CartPoleParams:
    ...
    def validate(self) -> bool:
        ...
        if any(
            v <= 0
            for v in [self.m_cart, self.m_pole, self.length, self.I, self.F_max, self.L]
        ):
            return False
    ...
    def to_array(self) -> np.ndarray:
        ...
        return np.array(
            [self.m_cart, self.m_pole, self.length, self.I, self.F_max, self.L]
        )
...
class CartPoleDynamics:
    ...
    def dynamics(self, state: np.ndarray, u: float) -> np.ndarray:
        ...
        p_ddot = (
            u
            + ml * theta_dot**2 * sin_theta
            - self.params.m_pole * self.params.g * sin_theta * cos_theta
        ) / denom

        # Pole angular acceleration
        theta_ddot = (
            -u * cos_theta
            - ml * theta_dot**2 * sin_theta * cos_theta
            + m_total * self.params.g * sin_theta
        ) / (self.params.length * denom)
    ...
    def compute_jacobians(
        self, state: np.ndarray, u: float, eps: float = 1e-6
        ...
        for i in range(n_states):
            ...
            A[:, i] = (self.dynamics(state_plus, u) - self.dynamics(state_minus, u)) / (
                2 * eps
            )
    ...
    def discretize_jacobians(
        self, A_cont: np.ndarray, B_cont: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
